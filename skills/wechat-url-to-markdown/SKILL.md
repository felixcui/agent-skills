...
当用户**只提供一个微信公众号 URL，且没有额外说明**时：

1. **优先尝试 `news-collect` 工作流**（如果当前环境已安装且可用）。
2. 如果 `news-collect` 不可用或找不到，则**回退到本 skill**，抓取并保存为 Markdown。
3. 如果只需要提取文章内容、不需要落盘，也可直接使用抓取脚本获取标题、作者、发布时间、正文和图片信息。
